...
overall_PitchMean = aggregate(pitchdata[, 6], list(pitchdata$ID_unique), mean)
#change the column names 
setnames(overall_PitchMean, "Group.1", "ID_unique")
setnames(overall_PitchMean, "PitchMean", "Overall_PitchMean")

Pitch_data_new = merge(overall_PitchMean, pitchdata, by = "ID_unique")
...
overall_PitchSd = aggregate(pitchdata[, 7], list(pitchdata$ID_unique), mean)

#change the column names 
setnames(overall_PitchSd, "Group.1", "ID_unique")
setnames(overall_PitchSd, "PitchSD", "Overall_PitchSD")

Pitch_data_new = merge(overall_PitchSd, Pitch_data_new, by = "ID_unique")
...
myvars <- c(unique("diagnosis"),unique("ID_unique"))
newdata <- pitchdata[myvars]
d = merge(newdata,overall_PitchSd)
...
It seems that the outcome variable (standardized pitch SD) is long-tailed
...
```{r}
#Model of pitch with default prior from brm
pitchmodel=brm(Overall_PitchSD.s~diagnosis+(1|ID),data=Pitch_data_new,cores=2,chain=2,iter = 2000)
```
